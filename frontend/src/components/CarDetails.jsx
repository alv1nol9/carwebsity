    ...
    <div className="car-details-page">
      <div className="car-detail-main">
        ...
        <div className="car-info-section">
          ...
          <div className="car-price">KES {Number(car.price).toLocaleString()}</div>
          ...
          <div className="cta-row">
            <a href={`https://wa.me/254700000000?text=I want to enquire about the ${car.make} ${car.model}`} className="wa-btn" target="_blank" rel="noopener noreferrer">💬 Enquire via WhatsApp</a>
            <a href="tel:0700000000" className="call-btn">📞 Call now</a>
            <button className="add-cart-btn" onClick={handleAddToCart} style={{ marginLeft: 8 }}>Add to Cart</button>
            {isAdmin() && (
              <button className="delete-btn" onClick={handleDelete}>Delete</button>
            )}
            <button className="add-cart-btn" style={{ marginLeft: 8, background: '#1877F3', color: '#fff' }} onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`, '_blank')} title="Share on Facebook">
              <FaShareAlt style={{ marginRight: 6 }} />Share
            </button>
          </div>
          ...
          <div className="vehicle-details-table">
            ...
            <table>
              <tbody>
                <tr><td>Year of manufacture</td><td>{car.year}</td></tr>
                <tr><td>Mileage</td><td>{car.mileage ? `${Number(car.mileage).toLocaleString()} km` : '—'}</td></tr>
                <tr><td>Drive</td><td>{car.drive || '—'}</td></tr>
                <tr><td>Engine Size</td><td>{car.engineSize || '—'}</td></tr>
                <tr><td>Fuel Type</td><td>{car.fuelType || '—'}</td></tr>
                {car.registration && <tr><td>Registration</td><td>{car.registration}</td></tr>}
                {car.insurance && <tr><td>Insurance</td><td>{car.insurance}</td></tr>}
              </tbody>
            </table>
          </div>
